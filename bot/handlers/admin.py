...
async def check_admin(message: Message, is_admin: bool) -> bool:
    ...
    if not is_admin:
        await message.answer(get_text(settings.default_locale, 'access_denied'))
...
async def cmd_provider_status(message: Message, is_admin: bool = False):
    ...
    text = get_text(settings.default_locale, 'provider_status', statuses='\n'.join(status_lines))
...
async def cmd_alerts_on(message: Message, is_admin: bool = False):
    ...
    text = get_text(settings.default_locale, 'alerts_enabled')
    await message.answer(text)
...
async def cmd_alerts_off(message: Message, is_admin: bool = False):
    ...
    text = get_text(settings.default_locale, 'alerts_disabled')
    await message.answer(text)
...
async def cmd_health_reset(message: Message, is_admin: bool = False):
    ...
    alert_manager.reset()
    text = get_text(settings.default_locale, 'health_reset')
    await message.answer(text)
